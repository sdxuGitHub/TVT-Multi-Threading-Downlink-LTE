...
void phy_procedures_eNB_uespec_RX(PHY_VARS_eNB *eNB,eNB_rxtx_proc_t *proc,const relaying_type_t r_type)
{
  ...
  uint16_t temp_rb=0;
/**************************************************/ 
  //初始化RX Global 參數 && Msg3 information
  eNB->flag_pucch = 0;
  eNB->flag_pusch = 0;
  eNB->flag_turbo = 0; 
  eNB->flag_viterbi = 0;
  ...
  for (i=0; i<NUMBER_OF_UE_MAX; i++) {
	  ...
	  //Table(eNB->active_UE) of doing PUSCH?...
	  if((eNB->ulsch[i]) &&
         (eNB->ulsch[i]->rnti>0) &&
         (eNB->ulsch[i]->harq_processes[harq_pid]->subframe_scheduling_flag==1)) {
			/*VCD_SIGNAL_DUMPER_DUMP_VARIABLE_BY_NAME(VCD_SIGNAL_DUMPER_VARIABLES_ENB_UL_mcs01+i,eNB->ulsch[i]->harq_processes[harq_pid]->mcs);
			VCD_SIGNAL_DUMPER_DUMP_VARIABLE_BY_NAME(VCD_SIGNAL_DUMPER_VARIABLES_ENB_UL_C01+i,eNB->ulsch[i]->harq_processes[harq_pid]->C);
			temp_rb+=eNB->ulsch[i]->harq_processes[harq_pid]->nb_rb;
			VCD_SIGNAL_DUMPER_DUMP_VARIABLE_BY_NAME(VCD_SIGNAL_DUMPER_VARIABLES_ENB_UL_RB01+i,eNB->ulsch[i]->harq_processes[harq_pid]->nb_rb);
			*/eNB->active_UE[i] = 1;
	  }
	  else {
		/*VCD_SIGNAL_DUMPER_DUMP_VARIABLE_BY_NAME(VCD_SIGNAL_DUMPER_VARIABLES_ENB_UL_mcs01+i,0);
		VCD_SIGNAL_DUMPER_DUMP_VARIABLE_BY_NAME(VCD_SIGNAL_DUMPER_VARIABLES_ENB_UL_C01+i,0);
		VCD_SIGNAL_DUMPER_DUMP_VARIABLE_BY_NAME(VCD_SIGNAL_DUMPER_VARIABLES_ENB_UL_RB01+i,0);*/
		eNB->active_UE[i] = 0;
	  }
	  eNB->ret[i] = 0;
	  eNB->Q_CQI[i] = 0;
  ...
  /*if(fp->N_RB_DL==25)
  {
	if(temp_rb>=10) VCD_SIGNAL_DUMPER_DUMP_FUNCTION_BY_NAME(VCD_SIGNAL_DUMPER_FUNCTIONS_more_than_10RB,1); 
  }
  else if(fp->N_RB_DL==100)
  {
	if(temp_rb>=20) VCD_SIGNAL_DUMPER_DUMP_FUNCTION_BY_NAME(VCD_SIGNAL_DUMPER_FUNCTIONS_more_than_10RB,1); 
  }*/
/**************************************************/ 
...
while(complete_decoding!=NUMBER_OF_UE_MAX){
  for (i=0; i<NUMBER_OF_UE_MAX; i++) {
	uint32_t temp = eNB->complete_turbo&(1<<i);
	if(eNB->active_UE[i]==0&&wait_decoding[i]==1){
		wait_decoding[i] = 0;
	...
	else if(eNB->active_UE[i]==1&&temp!=0&&wait_decoding[i]==1){
		wait_decoding[i] = 0;
		complete_decoding = complete_decoding+1;
		/*if ((eNB->ulsch[i]) &&
			(eNB->ulsch[i]->rnti>0) &&
...
//VCD_SIGNAL_DUMPER_DUMP_FUNCTION_BY_NAME(VCD_SIGNAL_DUMPER_FUNCTIONS_p2m,0);
  ...
  VCD_SIGNAL_DUMPER_DUMP_FUNCTION_BY_NAME(VCD_SIGNAL_DUMPER_FUNCTIONS_more_than_10RB,0);
  if (eNB->abstraction_flag == 0) {
    lte_eNB_I0_measurements(eNB,
			    subframe,
			    0,
  ...
  while(eNB->complete_turbo!=0xFFFFFFFF);
#endif
