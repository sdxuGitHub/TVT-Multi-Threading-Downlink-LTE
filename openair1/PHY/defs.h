...
typedef struct{
	pthread_t pthread_ofdm;
	pthread_cond_t cond_tx;
	pthread_mutex_t mutex_tx;
	pthread_attr_t attr_ofdm;
	//=====//
}ofdm;
...
typedef struct{
	...
	pthread_attr_t attr_ucch;
}ul_pucch;
typedef struct{
	pthread_t pthread_rx;
	pthread_cond_t cond_rx;
	pthread_mutex_t mutex_rx;
	pthread_attr_t attr_usch;
}ul_pusch;
typedef struct{
	pthread_t pthread_rx;
	pthread_cond_t cond_rx;
	pthread_mutex_t mutex_rx;
	pthread_attr_t attr_viterbi;
}viterbi;
typedef struct{
	int id;
	volatile int flag_wait;
...
typedef struct PHY_VARS_eNB_s {
  ...
  multi_share_channel thread_g[NUM_THREAD];
  ofdm thread_ofdm;
  
  volatile uint8_t ue_status[NUMBER_OF_UE_MAX];
  pthread_mutex_t uestatus_mutex;
  //----------------Part of RX procedure----------------
  ul_pucch thread_pucch;
  ul_pusch thread_pusch;
  viterbi thread_viterbi;
  turbo thread_turbo[NUM_THREAD];
  
  volatile uint32_t turbo_status;
  //volatile uint8_t turbo_status[NUMBER_OF_UE_MAX];
  pthread_mutex_t turbo_status_mutex;
  pthread_mutex_t complete_turbo_mutex;
   
  //壓力測試
   LTE_eNB_ULSCH_t *ulsch_t[NUMBER_OF_UE_MAX+1]; 
  //----------------Part of TX procedure----------------
  //FLAG
  volatile uint8_t flag_m2p;
  volatile uint8_t flag_cch;
  volatile uint8_t flag_pdsch;
  volatile uint8_t flag_sch;
  volatile uint8_t flag_ofdm;
  ...
  volatile uint8_t complete_pdsch;
  
  pthread_t pthread_TX_timing_thread;
  
  int temp;//10
  double timing[MAX_OF_TIMING];
  //----------------Part of RX procedure----------------
  //Decoding Parameter
  volatile int ret[NUMBER_OF_UE_MAX];
  volatile unsigned int Q_CQI[NUMBER_OF_UE_MAX];
  volatile unsigned int iind[NUMBER_OF_UE_MAX][MAX_NUM_ULSCH_SEGMENTS];//iind要再check MAX_NUM_ULSCH_SEGMENTS是否夠大?
  volatile uint8_t active_UE[NUMBER_OF_UE_MAX];
  //FLAG
  volatile uint8_t flag_pucch;
  volatile uint8_t flag_pusch;
  volatile uint8_t flag_viterbi;
  volatile uint8_t flag_turbo;
  //volatile int flag_UL_copy;
  //DONE
  volatile uint8_t complete_pucch;
  volatile uint8_t complete_pusch[NUMBER_OF_UE_MAX];
  volatile uint8_t complete_viterbi_parameter[NUMBER_OF_UE_MAX];
  volatile uint8_t complete_turbo_parameter[NUMBER_OF_UE_MAX];//***Unused***
  volatile uint8_t complete_viterbi_thread;
  volatile uint8_t complete_turbo_thread;
  volatile uint8_t complete_viterbi[NUMBER_OF_UE_MAX];
  volatile uint32_t complete_turbo;
  //volatile uint8_t complete_turbo[NUMBER_OF_UE_MAX];
  /*struct timespec tt11, tt12, tt13, tt14,tt15, tt16, tt17, tt18;
  struct timespec tt21, tt22;
  struct timespec tt31, tt32, tt33, tt34,tt35, tt36, tt37, tt38;
  struct timespec tt41, tt42;*/
  pthread_t pthread_RX_timing_thread;
  
  //RX_pressuretest
  LTE_eNB_ULSCH_t  *ulsch_p[NUMBER_OF_UE_MAX];  
} PHY_VARS_eNB;
