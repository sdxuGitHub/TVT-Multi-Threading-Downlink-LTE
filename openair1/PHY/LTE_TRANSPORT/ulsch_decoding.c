...
int ulsch_decoding_data(PHY_VARS_eNB *eNB,int UE_id,int harq_pid,int llr8_flag) {
  ...
  int16_t dummy_w[MAX_NUM_ULSCH_SEGMENTS][3*(6144+64)];
  LTE_eNB_ULSCH_t *ulsch;
#ifndef RX_pressuretest
  ulsch = eNB->ulsch[UE_id];
#else
  LTE_eNB_ULSCH_t *ulsch_p;
  LTE_UL_eNB_HARQ_t *ulsch_harq_p;
  if(UE_id<NUMBER_OF_UE_MAX) ulsch = eNB->ulsch[UE_id];
  else {
	  ulsch = eNB->ulsch[0];
	  ulsch_p = eNB->ulsch_p[UE_id-NUMBER_OF_UE_MAX];
	  ulsch_harq_p = ulsch_p->harq_processes[harq_pid];
  }
#endif
  /*LTE_eNB_ULSCH_t *ulsch;
...
/**********************************RX_pressuretest?*********************************/
  ...
  // Reassembly of Transport block here
	...
	if(UE_id<NUMBER_OF_UE_MAX){
	...
	else{//RX_pressuretest fake
		iind = eNB->iind[0][r];
		
		ret = tc(&ulsch_harq->d[r][96],
			 ulsch_harq_p->c[r],//RX_pressuretest fake
			 Kr,
			 f1f2mat_old[iind*2],
			 f1f2mat_old[(iind*2)+1],
			 ulsch->max_turbo_iterations,//MAX_TURBO_ITERATIONS,
	  ...
	  // Reassembly of Transport block here
		...
		Kr = ulsch_harq->Kplus;
		  ...
		  if (r==0) {
		memcpy(ulsch_harq_p->b,
			   &ulsch_harq_p->c[0][(ulsch_harq->F>>3)],
			   Kr_bytes - (ulsch_harq->F>>3) - ((ulsch_harq->C>1)?3:0));
		offset = Kr_bytes - (ulsch_harq->F>>3) - ((ulsch_harq->C>1)?3:0);
		  } else {
		memcpy(ulsch_harq_p->b+offset,
			   ulsch_harq_p->c[r],
			   Kr_bytes - ((ulsch_harq->C>1)?3:0));
		offset += (Kr_bytes- ((ulsch_harq->C>1)?3:0));
		  }
...
unsigned int  ulsch_decoding(PHY_VARS_eNB *eNB,eNB_rxtx_proc_t *proc,
                             uint8_t UE_id,//Use this
                             uint8_t control_only_flag,
                             uint8_t no1,
                             uint8_t no2)
{
  VCD_SIGNAL_DUMPER_DUMP_FUNCTION_BY_NAME(VCD_SIGNAL_DUMPER_FUNCTIONS_decoding_para01+UE_id,1);
  int16_t *ulsch_llr;
  ...
  //uint8_t complete_pusch=0,wait_pusch[NUMBER_OF_UE_MAX];
  //for(UE_id=0;UE_id<NUMBER_OF_UE_MAX;UE_id++) wait_pusch[UE_id] = 1;
  
  harq_pid = subframe2harq_pid(frame_parms,proc->frame_rx,subframe);
  VCD_SIGNAL_DUMPER_DUMP_FUNCTION_BY_NAME(VCD_SIGNAL_DUMPER_FUNCTIONS_PHY_ENB_ULSCH_DECODING0+harq_pid,1);
/*  
  //Trigger viterbi & turbo and use busy waiting to get jobs
  eNB->flag_viterbi = 1;
...
*/
/*while(complete_pusch!=NUMBER_OF_UE_MAX){
 if(oai_exit) break;
 for(UE_id=0;UE_id<NUMBER_OF_UE_MAX;++UE_id){
   if(eNB->complete_pusch[UE_id]==1&&wait_pusch[UE_id]==1){
	  wait_pusch[UE_id] = 0;
	  complete_pusch = complete_pusch+1;*/
	  /*if ((eNB->ulsch[UE_id]) &&
          (eNB->ulsch[UE_id]->rnti>0) &&
          (eNB->ulsch[UE_id]->harq_processes[harq_pid]->subframe_scheduling_flag==1)){*/
			  
  uint8_t Nbundled = eNB->ulsch[UE_id]->harq_processes[harq_pid]->V_UL_DAI;
  //uint8_t llr8_flag = eNB->ulsch[UE_id]->harq_processes[harq_pid]->nb_rb>20 ? 1 : 0;
  ulsch_llr = eNB->pusch_vars[UE_id]->llr;
  ulsch = eNB->ulsch[UE_id];
  ...
  for (i=0; i<((Hpp*Q_m)>>5); i++) {
    ...
    */
    ...
    ((__m128i*)cseq)[i2++] = ((__m128i*)unscrambling_lut)[1+((s&65535)<<1)];
#else
	//printf("\n %d %d %d\n",s&65535,((__m256i*)cseq)[0],((__m256i*)unscrambling_lut)[s&65535]);
...
VCD_SIGNAL_DUMPER_DUMP_FUNCTION_BY_NAME(VCD_SIGNAL_DUMPER_FUNCTIONS_decoding_para01+UE_id,0);
  eNB->Q_CQI[UE_id] = Q_CQI;
  //eNB->complete_viterbi_parameter[UE_id] = 1;
  //  printf("before cqi c[%d] = %p\n",0,ulsch_harq->c[0]);
  if (Q_CQI>0) {
	VCD_SIGNAL_DUMPER_DUMP_FUNCTION_BY_NAME(VCD_SIGNAL_DUMPER_FUNCTIONS_viterbi01+UE_id,1);
	...
	VCD_SIGNAL_DUMPER_DUMP_FUNCTION_BY_NAME(VCD_SIGNAL_DUMPER_FUNCTIONS_viterbi01+UE_id,0);
#ifdef DEBUG_ULSCH_DECODING
  ...
  //Over viterbi/turbo decoder參數處理
  unsigned int r_offset=0,Kr_bytes,iind;
  int16_t dummy_w[MAX_NUM_ULSCH_SEGMENTS][3*(6144+64)];
  unsigned int E;
/*-----------------------turbo rate matching & deinterleaving/*-----------------------*/
	for (r=0; r<ulsch_harq->C; r++) {
		// printf("before subblock deinterleaving c[%d] = %p\n",r,ulsch_harq->c[r]);
		// Get Turbo interleaver parameters
		if (r<ulsch_harq->Cminus)
	...
	eNB->complete_turbo_parameter[UE_id] = 1;
/*-----------------------Over turbo rate matching & deinterleaving/*-----------------------*/
